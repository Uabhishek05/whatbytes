  ...
  return (
    <main className="w-full px-4 py-5 sm:px-6 sm:py-8 lg:px-8 xl:px-10">
      ...
      <section className="mt-5 grid gap-5 lg:mt-6 lg:grid-cols-[1fr_0.9fr] lg:gap-8">
        <div className="animate-rise motion-card rounded-xl bg-white p-4 shadow-sm sm:p-6">
          <div className="product-image-bg aspect-square overflow-hidden rounded-xl">
            <img
              src={product.image}
              alt={product.title}
              className="image-zoom h-full w-full object-contain p-4 sm:p-8"
            />
          </div>
          <div className="mt-4 grid gap-2 sm:grid-cols-3 sm:gap-3">
            {product.specs.map((spec) => (
              <div
                key={spec}
                className="rounded-lg border border-line bg-mist px-3 py-3 text-center text-xs font-bold text-ink transition duration-200 hover:border-[#0068c9] hover:bg-white"
              >
                {spec}
              </div>
            ))}
        ...
        <div className="animate-rise motion-card rounded-xl bg-white p-4 shadow-sm sm:p-7" style={{ animationDelay: "90ms" }}>
          ...
          <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center">
            <div className="flex h-12 items-center overflow-hidden rounded-lg border border-line bg-white">
              <button
                ...
                className="motion-button grid h-12 w-12 place-items-center text-ink hover:bg-mist"
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            <div className="motion-card flex items-center gap-3 rounded-lg border border-line p-4">
      ...
      <section className="animate-rise mt-6 rounded-xl bg-white p-4 shadow-sm sm:mt-8 sm:p-7" style={{ animationDelay: "140ms" }}>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {["Excellent quality and quick delivery.", "Looks premium and works exactly as expected."].map(
            (review) => (
              <article key={review} className="motion-card rounded-lg border border-line p-4">
