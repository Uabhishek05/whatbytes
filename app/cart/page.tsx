...
export default function CartPage() {
  ...
  return (
    <main className="px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
      ...
      {items.length === 0 ? (
        <section className="rounded-xl bg-white p-6 text-center shadow-sm sm:p-12">
        ...
        <section className="grid gap-7 lg:grid-cols-[1fr_340px]">
          <div className="space-y-4">
            {items.map((item) => (
              <article
                key={item.product.id}
                className="grid grid-cols-[92px_1fr] gap-3 rounded-xl bg-white p-3 shadow-sm sm:grid-cols-[120px_1fr_auto] sm:gap-4 sm:p-4"
              >
                <Link
                ...
                <div className="col-span-2 flex items-center justify-between gap-3 sm:col-span-1 sm:flex-col sm:items-end sm:justify-between">
                  ...
                  <button
                    ...
                    className="grid h-10 w-10 place-items-center rounded-lg border border-line text-coral hover:bg-red-50"
          ...
          <aside className="h-fit rounded-xl bg-white p-4 shadow-sm sm:p-6">
            ...
            <button className="mt-6 h-12 w-full rounded-lg bg-[#0068c9] font-bold text-white hover:bg-[#0056aa]">
              Checkout
            </button>
            <button
              onClick={clearCart}
              className="mt-3 h-11 w-full rounded-lg border border-line font-bold text-ink hover:bg-mist"
            >
              Clear Cart
            </button>
          </aside>
